...
function RotatingPhrase() {
  ...
  return (
    <span className="relative grid sm:inline-grid max-w-full align-bottom">
      {/* Invisible sizers: every phrase occupies the same cell, so the reserved
          height/width always fits the tallest & widest phrase — zero layout shift. */}
      {PHRASES.map((p) => (
        <span
          key={p}
          aria-hidden
          className="col-start-1 row-start-1 invisible min-w-0 text-gradient-gold italic font-normal"
        >
          {p}
        </span>
      ))}
